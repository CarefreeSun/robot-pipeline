...
def call_models(instance_data, model_vq: VQGANVisionActionEval, tokenizer: LlamaTokenizer, vla_pipe: MistralInVisionActionFeatMask,  
    ...
    output_vision_tokens_pred, output_action_tokens_pred, output_clip_description_pred = call_vla(instance_data, video_tokens, action_tokens, tokenizer, vla_pipe, data_args, device)

    output_vision_pred = model_vq.decode_video(output_vision_tokens_pred).squeeze(0).permute(1,0,2,3).detach().cpu() # 6, 3, 256, 256
    ...
    instance_data['vision'] = output_vision_pred
